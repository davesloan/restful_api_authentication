...
describe 'Authentication', type: :request, requests: true do
  let(:path) { '/help/master_authentication' }
  let!(:rest_client) { FactoryGirl.create(:rest_client, is_master: true) }
  let(:headers) { {} }
  ...
  context 'unauthorized' do
    ...
    it 'should return 401 (unauthorized) when requesting without master permissions' do
      rest_client.is_master = false
      rest_client.save
      headers = signiture_headers(rest_client)
      get path, params: nil, headers: headers
      expect(response).to have_http_status 401
      expect(json[:errors][0]).to eq('client does not have the required permissions')
    end
